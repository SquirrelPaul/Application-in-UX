...
def parText(result,text):
    ...
    name_search = text.find("div", class_='detail').find('a',class_='hoverinfo_trigger fs14 fw-b')
    if name_search:
    ...
    AnimeGrade_search = text.find("td",class_='score ac fs14').find("span",class_='text on')
    if AnimeGrade_search:
        AnimeGrade = AnimeGrade_search.get_text()
    
    result.append((rank, Animename, (Animeinfo), AnimeGrade))

pages = 300
result = []
url0 = "https://myanimelist.net/topmanga.php?limit="
url = ""
#主程序，循环
...
pd.DataFrame(result, columns=['排名','漫画名称','漫画信息', '漫画评分']).to_excel('E:\MAL2020_Manga2.xlsx', index=None, encoding='utf-8')  # 【指定字段写入的规则】以sep的属性作为分隔符，将文件写入txt文件中
